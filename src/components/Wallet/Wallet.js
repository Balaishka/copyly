...
import { testWallet } from "../../configs/constants";
...
function Wallet({ t, addZero, recordingData }) {
  const [lastActivity, setLastActivity] = useState("");
  const [lastTime, setLastTime] = useState(0);
  const [isSign, setIsSign] = useState(false);

  useEffect(() => {
    const date = new Date(testWallet.last_activity * 1000);
    ...
    if (diffDay || diffHours) {
        setLastTime(`${diffDay ? diffDay + " " + t("day") + " ":""}${diffHours ? diffHours + " " + t("hours") + " ":""}${t("ago")}`);
    }
  }, []);
  ...
  return (
    <div className="wallet">
        ...
        <div className="wallet__header-container">
          <div className="wallet__header-info">
            ...
            <div className="wallet__num">
              <h1 className="wallet__title">{testWallet.num}</h1>
              <CopyToClipboard text={testWallet.num}>
                <svg
          ...
          <p className="wallet__last-activity">
            <span>{t("wallet_last_activity")}:</span> <span>{lastActivity}{" "}
            {lastTime.length !== 0 && <>({lastTime})</>}</span>
          </p>
        </div>
      ...
      <section className="wallet__info">
        <Fact title={t("balance")} info={testWallet.cur_balance} />
        <Fact title={t("table_th_2")} info={testWallet.pnl} />
        <Fact title={t("table_th_3")} info={testWallet.profit_factor} />

        <Fact
          title={t("succesfull")}
          info={testWallet.win_rate_amount}
          type="desctop"
        />
        <Fact
          title={t("amount")}
          info={testWallet.overall_tokens}
          type="desctop"
        />

        <Fact title={t("profit")} info={testWallet.profits} type="mobile" />
        <Fact title={t("losts")} info={testWallet.losses} type="mobile" />
        <Fact
          title={t("profit_factor")}
          info={testWallet.profit_factor}
          type="mobile"
        />
      </section>
      ...
      <section className="wallet__graphs">
        <div className="wallet__left">
            <Balance recordingData={recordingData} title={t("graph_balance")} />
        ...
        <div className="wallet__right">
            ...
            <div className="wallet__info wallet__info_type_bottom">
                ...
                <Fact title={t("losts")} info={testWallet.losses} type="desctop" />
                <Fact
                title={t("profit_factor")}
                info={testWallet.profit_factor}
                type="desctop"
